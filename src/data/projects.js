export const projects = [
  {
    num: '01',
    title: 'MathIQ',
    description: 'My Software as a Service. Math is one of my passions so this one was a no-brainer. I think anyone can master mathematics given a good teacher. The commits tell a detailed story of this project.',
    repoPath: 'thoma-sH/MathIQ',
    previewSrc: '/mathiq/mathiq.gif',
    previewAspect: 'desktop',
    meta: { label: 'SAAS', status: 'LIVE', glowing: true },
  },
  {
    num: '02',
    title: 'Lacuna',
    description: "A social media app I'm currently building. Still rough around the edges, still figuring it out.",
    repoPath: 'thoma-sH/Lacuna',
    previewSrc: '/lacuna/lacuna.gif',
    previewAspect: 'phone',
    meta: { label: 'SOCIAL', status: 'IN PROGRESS' },
  },
  {
    num: '03',
    title: 'Concert Companion',
    description: "A 24-hour build from Hog Hacks 2026 — a companion app for live shows. We've kept iterating on it since.",
    repoPath: 'thoma-sH/concert-companion',
    previewSrc: '/concert_companion/cc.gif',
    previewAspect: 'desktop',
    meta: { label: "HOG HACKS '26" },
  },
  {
    num: '04',
    title: 'Java Mini-Game',
    description: 'A Link-inspired adventure I built in Java for my Programming Paradigms class.',
    repoPath: 'thoma-sH/javaminigame-Link_from_Paradigms',
    previewSrc: '/link/link.gif',
    previewAspect: 'desktop',
    meta: { label: 'COURSEWORK · JAVA ↗' },
  },
  {
    num: '05',
    title: 'Photon',
    description: 'Modernized Laser Tag software for the Original Photon Laser Tag. This was a collaborative endeavor with three other peers. I developed the GUI and oversaw pull requests and the final product.',
    repoPath: 'thoma-sH/photon-main',
    previewSrc: '/photon/image12.gif',
    previewAspect: 'desktop',
    meta: { label: 'TEAM 20 · PYTHON ↗' },
  },
];

export const sections = [
  { id: 'intro', label: 'INTRO' },
  { id: 'work', label: 'WORK' },
  { id: 'profile', label: 'PROFILE' },
  { id: 'about', label: 'ABOUT' },
  { id: 'thanks', label: 'THANKS' },
];

export const stack = ['TypeScript', 'Python', 'React', 'Durable Objects'];

export const skills = ['Java', 'JavaScript', 'Python', 'C++', 'Dart', 'HTML / CSS', 'Git', 'Agile SDLC', 'CI/CD'];
